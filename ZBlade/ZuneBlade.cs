...
namespace ZBlade
{
	...
	public partial class ZuneBlade : Microsoft.Xna.Framework.DrawableGameComponent
	{
		...
		private ResourceContentManager content = null;
		...
		protected override void LoadContent()
		{
			...
			font_10 = content.Load<SpriteFont>("Segoe_UI_10");
			font_12 = content.Load<SpriteFont>("Segoe_UI_12");
			font_14 = content.Load<SpriteFont>("Segoe_UI_14");

            bgGlossTex = content.Load<Texture2D>("bgBottom");
            bladeGloss = content.Load<Texture2D>("bladeGloss");
            bladeBorder = content.Load<Texture2D>("bladeBorder");
			menuChoiceTex = content.Load<Texture2D>("menuchoice");
			menuHighlightTex = content.Load<Texture2D>("menuhighlight");
			menuArrowTex = content.Load<Texture2D>("menuArrow");
			progressbarOverlay = content.Load<Texture2D>("progressbarOverlay");

            whitePixel = new Texture2D(GraphicsDevice, 1, 1);
            whitePixel.SetData<uint>(new uint[] {0xffffffff });

            Key1 = content.Load<Texture2D>("key1");
            Key2 = content.Load<Texture2D>("key2");
            Key6 = content.Load<Texture2D>("key6");
				...
				for (int x = menuTopIndex; x < currentMenu.Nodes.Count && x < menuBottomIndex; x++)
				{
					...
					if (x == CurrentMenuItem)
						batch.Draw(
							...
							new Color(255, 255, 255, (byte)selectionFade.Position.X));
				}
